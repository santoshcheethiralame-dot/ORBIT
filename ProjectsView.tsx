...
function ProjectCard({ project, subject, onLog, onEdit, onDelete, onToggleComplete }: {
  ...
  return (
    <div className={`relative rounded-2xl border overflow-hidden transition-all duration-300 group ${
      project.completed ? "bg-white/[0.015] border-white/[0.04] opacity-60" : "bg-white/[0.03] border-white/[0.08] hover:border-white/[0.14]"
    }`}>
      {/* Left accent bar */}
      {!project.completed && <div className={`absolute left-0 top-0 bottom-0 w-[3px] ${priority.dot}`}/>}

      <div className="pl-5 pr-4 pt-4 pb-4">
        {/* Top row */}
        <div className="flex items-start gap-3 mb-3">
          <div className="flex-1 min-w-0">
            <div className={`inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md mb-2 ${cc.text} bg-white/[0.05]`}>
              <div className={`w-1.5 h-1.5 rounded-full ${cc.bg}`}/>{subject?.name ?? "Unknown"}
            </div>
            <h3 className={`font-semibold text-[14px] leading-snug ${project.completed ? "line-through text-zinc-600" : "text-white"}`}>
              {project.name}
            </h3>
          </div>
          <div className="flex items-center gap-1 shrink-0">
            {!project.completed && (
              <button onClick={onLog}
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-indigo-600/20 hover:bg-indigo-600/40 text-indigo-300 text-[11px] font-bold transition-all border border-indigo-500/20 hover:border-indigo-400/40">
                <LogIn size={11}/>Log
              </button>
            )}
            <div className="relative">
              <button onClick={() => setMenuOpen(o=>!o)}
                className="p-1.5 rounded-lg text-zinc-600 hover:text-zinc-300 hover:bg-white/10 transition-all">
                <MoreHorizontal size={14}/>
              </button>
              {menuOpen && (
                <div className="absolute right-0 top-full mt-1 w-36 bg-zinc-900 border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden">
                  <button onClick={()=>{onEdit();setMenuOpen(false);}} className="w-full flex items-center gap-2 px-3 py-2.5 text-[11px] text-zinc-300 hover:bg-white/10 hover:text-white transition-all">
                    <Edit2 size={11}/>Edit
                  </button>
                  <button onClick={()=>{onToggleComplete();setMenuOpen(false);}} className="w-full flex items-center gap-2 px-3 py-2.5 text-[11px] text-zinc-300 hover:bg-white/10 hover:text-white transition-all">
                    {project.completed ? <><Circle size={11}/>Reopen</> : <><Check size={11}/>Mark done</>}
                  </button>
                  <div className="h-px bg-white/[0.06] mx-2"/>
                  <button onClick={()=>{onDelete();setMenuOpen(false);}} className="w-full flex items-center gap-2 px-3 py-2.5 text-[11px] text-red-400 hover:bg-red-500/10 transition-all">
        ...
        <div className="mb-3">
          <div className="flex items-center justify-between text-[11px] mb-1.5">
            <span className="text-zinc-500">{fmtMins(project.completedEffortMinutes)} done</span>
            <span className={`font-bold ${pct>=100?"text-emerald-400":pct>=60?"text-indigo-300":"text-zinc-400"}`}>{pct}%</span>
            <span className="text-zinc-600">{fmtMins(remaining)} left</span>
          </div>
        ...
        <div className="flex items-center gap-3 flex-wrap text-[11px]">
          ...
          <span className={`flex items-center gap-1 ${priority.text}`}>
            <div className={`w-1.5 h-1.5 rounded-full ${priority.dot}`}/>{priority.label}
          </span>
          {project.completed && (
            <span className="flex items-center gap-1 text-emerald-500 ml-auto font-medium">
  ...
  return (
    <div className="pb-32 pt-8 px-4 lg:px-8 w-full max-w-5xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          { icon: FolderKanban, label:"Active",     value: active.length,          clr:"text-indigo-400", bg:"bg-indigo-500/10" },
          { icon: Clock,        label:"Hours left",  value:`${(totalLeft/60).toFixed(1)}h`, clr:"text-cyan-400",   bg:"bg-cyan-500/10"   },
          { icon: Flame,        label:"High/Urgent", value: urgent.length,          clr:"text-amber-400",  bg:"bg-amber-500/10"  },
          { icon: AlertTriangle,label:"Overdue",     value: overdue.length,
            clr: overdue.length>0?"text-red-400":"text-zinc-600",
            bg:  overdue.length>0?"bg-red-500/10":"bg-white/[0.03]" },
        ].map(({icon:Icon,label,value,clr,bg})=>(
          <div key={label} className="rounded-xl bg-white/[0.03] border border-white/[0.06] px-4 py-3.5 flex items-center gap-3">
            <div className={`w-9 h-9 rounded-lg ${bg} flex items-center justify-center shrink-0`}>
              <Icon size={16} className={clr}/>
            </div>
            <div>
              <div className="text-[10px] text-zinc-500 uppercase tracking-wider font-bold">{label}</div>
              <div className={`text-xl font-bold font-mono ${clr}`}>{value}</div>
            </div>
          </div>
        ))}
      </div>
